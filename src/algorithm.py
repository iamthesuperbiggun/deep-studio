...
Z-Algorithm (zalgo)
---------------------------------
This module implements linear-time pattern preprocessing for substring search.
...
class ZalgoResult:
    ...
    def __repr__(self) -> str:
        return f"<ZalgoResult result={self.result!r} meta={self.meta}>"

def core_algorithm(data: Any, **kwargs: Any) -> ZalgoResult:
    """
    Core algorithm stub.
    Parameters
    ----------
    data : Any
        Input structure (linear-time pattern preprocessing for substring search).
    kwargs : Any
    ...
    return ZalgoResult(result={"acc": acc, "trace": trace}, meta=meta)
...
def pretty_print(result: ZalgoResult) -> str:
    """Create a human-readable summary."""
    acc = result.result.get("acc")
    pts = result.meta.get("trace_points")
    return f"Z-Algorithm: acc={acc:.4f}, trace_points={pts}"
